...
import { OutlineCloseIcon } from '../../ui-components/CustomIcons';
import CancelBtn from '../Settings/CancelBtn';
import AddBtn from '../Settings/Owners/AddBtn';
import { useActiveMultisigContext } from '../../context/ActiveMultisigContext';
...
import { IAddressBookItem, ISharedAddressBooks, NotificationStatus } from '../../types';
		...
		<>
			...
			>
				<Form className='my-0 w-[560px]'>
					<p className='text-white font-medium text-sm leading-[15px]'>This will update the Address book for all Signatories of this Multisig, would you like to continue?</p>
	...
	const handleSharedAddressBookUpdate = async () => {
		try {
			...
			if (!userAddress || !signature) {
				...
				const addAddressRes = await fetch(`${FIREBASE_FUNCTIONS_URL}/updateSharedAddressBookEth`, {
					body: JSON.stringify({
						address: addressToEdit,
						discord,
						email,
				...
				if (addAddressData) {
					...
					const updateIndex = copyAddressBook.findIndex((item) => item.address === addressToEdit);
					if (updateIndex > -1) {
	...
	return (
		<>
			...
			>
				...
				{personalToShared ? (
					<Form className='my-0 w-[560px]'>
						<p className='text-white font-medium text-sm leading-[15px]'>This will update the Address Book for All Signatories, do you want to proceed?</p>
					</Form>
				) : (
					<Form
