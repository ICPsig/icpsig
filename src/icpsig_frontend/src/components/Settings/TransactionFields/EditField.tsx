			...
			<>
				...
				>
					...
					<section className='flex items-center gap-x-5 justify-between mt-10'>
						...
						/>
						<ModalBtn
							disabled={!name || (name === dropdownState?.[openEditOptionModal.i].optionName && archieved === !!dropdownState?.[openEditOptionModal.i].archieved)}
							onClick={() => {
		...
		<>
			...
			>
				<div className={className}>
					...
					<div className=' p-[10px] mb-4 text-text_secondary border-2 border-dashed border-bg-secondary rounded-lg'>{subfieldType}</div>
					<Form disabled={loading}>
												...
												<AutoComplete
													...
													notFoundContent={
														<Button
															...
															onClick={() => {
																setDropdownState((prev) => (prev ? [...prev, { optionName: newOption }] : [{ optionName: newOption }]));
																setNewOption('');
															}}
						...
						<section className='flex items-center gap-x-5 justify-between mt-10'>
							...
							/>
							<ModalBtn
								disabled={dropdownOptions && dropdownState === dropdownOptions && requiredState === required && (newSubfieldName === subfieldName || newSubfieldName === '')}
								loading={loading}
								onClick={handleSave}
								className='w-[200px]'
								title='Save'
