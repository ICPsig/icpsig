...
const RemoveOwner = ({
	...
	const { multisigAddresses, activeMultisig, address, identityBackend } = useGlobalUserDetailsContext();
	...
	const handleRemoveOwner = async () => {
		setLoading(true);
		try {
			const safeTxHash = (await identityBackend.createRemoveOwnerTx(activeMultisig, addressToRemove, newThreshold)).data;
			if (safeTxHash) {
				onCancel?.();
				setLoading(false);
				queueNotification({
							...
							<p className='flex items-center justify-center gap-x-[16.83px] p-[12.83px] bg-bg-secondary rounded-lg'>
								...
								<Tooltip title={newThreshold === oldSignatoriesLength - 1 && 'Threshold must be Less than or Equal to Signatories'}>
									<Button
										...
										className={`p-0 outline-none border rounded-full flex items-center justify-center ${
											newThreshold === oldSignatoriesLength - 1 ? 'border-text_secondary text-text_secondary' : 'text-primary border-primary'
										} w-[14.5px] h-[14.5px]`}
									>
										+
									</Button>
