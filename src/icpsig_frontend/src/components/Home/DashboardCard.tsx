			...
			<div className={`${className} relative bg-bg-main flex flex-col justify-between rounded-lg p-5 shadow-lg h-[17rem] scale-90 w-[111%] origin-top-left`}>
				...
				<div className='w-full'>
					<div className='flex gap-x-3 items-center'>
						<div className='relative'>
							...
							<div className={' bg-primary text-white text-sm rounded-lg absolute -bottom-1 left-[16px] px-2'}>
								...
								{activeMultisigData?.signatories?.length ? activeMultisigData?.signatories?.length : currentMultisig?.signatories.length}
							</div>
						</div>
						<div>
                  ...
                  <button
                    onClick={() =>
                      openModal(
                        "Address QR",
                        <AddressQr address={currentMultisig.address} />
                      )
                    }
                  >
				...
				<div className='flex gap-x-5 flex-wrap text-xs'>
					...
					<div>
						...
						<div className='font-bold text-lg text-primary'>{!activeMultisigData?.safeBalance ? <Spin size='default' /> : <>{balance}</>}</div>
					</div>
					{/* <div>
