...
const TxnCard = () => {
	const { activeMultisig, address, identityBackend, addressBook } = useGlobalUserDetailsContext();
							...
							completedTransactions.length > 0 ? (
								completedTransactions
									...
									.map((transaction, i) => {
										...
										const sent =
											transaction.type === 'sent' || transaction.type === 'MULTISIG_TRANSACTION' || transaction.type === 'removeOwner' || transaction.type === 'multiSend';
										...
										return (
											...
											>
												<div className='flex flex-1 items-center'>
													<div className={`${sent ? 'bg-failure' : 'bg-success'} bg-opacity-10 rounded-lg p-2 mr-3 h-[38px] w-[38px] flex items-center justify-center`}>
														<img
