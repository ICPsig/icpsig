...
import { CircleCheckIcon, CirclePlusIcon, CircleWatchIcon, CopyIcon } from 'src/ui-components/CustomIcons';
...
const SentInfo: FC<ISentInfoProps> = ({ approvals, amount, from, className, date, recipientAddress, callHash, note, loading, txType, addressAddOrRemove, transactionFields }) => {
	...
	return (
		<div className={classNames('flex gap-x-4', className)}>
			<article className='p-4 rounded-lg bg-bg-main flex-1'>
				...
				{addressAddOrRemove && (
					<div className='flex items-center gap-x-5 mt-3'>
						<span className='text-text_secondary font-normal text-sm leading-[15px]'>{txType === 'addOwnerWithThreshold' ? 'Added Owner' : 'Removed Owner'}:</span>
						<p className='flex items-center gap-x-3 font-normal text-xs leading-[13px] text-text_secondary'>
