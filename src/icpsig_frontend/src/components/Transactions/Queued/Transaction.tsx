...
const Transaction: FC<ITransactionProps> = ({ approvals, callData, callHash, date, threshold, notifications, value, onAfterApprove, onAfterExecute, txType, recipientAddress }) => {
	const { activeMultisig, address, identityBackend } = useGlobalUserDetailsContext();
	...
	const getTransactionDetails = useCallback(async () => {
		...
		const { data: getTransactionData, error: getTransactionErr } = (await getTransactionDetailsRes.json()) as {
			data: ITransaction;
			error: string;
		};
	...
	const handleApproveTransaction = async () => {
		setLoading(true);
		try {
			const response = await identityBackend.approveTransaction('', activeMultisig, callHash);
			if (response) {
				const updateTx = {
					signer: address,
					txHash: callHash,
					...
					header={
						getMultiDataLoading ? (
							...
							<div
								...
								className={classNames('grid items-center grid-cols-9 cursor-pointer text-white font-normal text-sm leading-[15px]')}
							>
								...
								<p className={`${txType === 'addOwnerWithThreshold' || txType === 'removeOwner' ? 'col-span-4' : 'col-span-2'} flex items-center justify-end gap-x-4`}>
									...
									<span className='text-white text-sm'>{transactionInfoVisible ? <CircleArrowUpIcon /> : <CircleArrowDownIcon />}</span>
								</p>
							</div>
						)
					}
				>
					...
					<div>
						...
						<SentInfo
							...
							addressAddOrRemove={
								txType === 'addOwnerWithThreshold' ? decodedCallData.parameters?.[0]?.value : txType === 'removeOwner' ? decodedCallData.parameters?.[1]?.value : ''
							}
							callHash={callHash}
