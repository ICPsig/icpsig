...
import { ArrowRightIcon, CircleCheckIcon, CirclePlusIcon, CircleWatchIcon, CopyIcon, EditIcon, OutlineCloseIcon } from 'src/ui-components/CustomIcons';
	...
	return (
		<div className={classNames('flex gap-x-4', className)}>
			...
			<article className='p-4 rounded-lg bg-bg-main flex-1'>
				...
				{!(txType === 'addOwnerWithThreshold' || txType === 'removeOwner') && <Divider className='bg-text_secondary my-5' />}
				...
				{addressAddOrRemove && (
					<div className='flex items-center gap-x-5 mt-3 justify-between'>
						<span className='text-text_secondary font-normal text-sm leading-[15px]'>{txType === 'addOwnerWithThreshold' ? 'Adding Owner' : 'Removing Owner'}:</span>
						<p className='flex items-center gap-x-3 font-normal text-xs leading-[13px] text-text_secondary'>
						...
						{!!transactionFields && Object.keys(transactionFields).length !== 0 && transactionFields.category !== 'none' && (
							<>
								<div className='flex items-center justify-between mt-3'>
									...
									<span className='text-primary border border-solid border-primary rounded-xl px-[6px] py-1'>{transactionFields?.category}</span>
								</div>
								{transactionFields &&
									...
									Object.keys(transactionFields?.subfields).map((key) => {
										...
										return (
											...
											>
												...
												<span className='text-waiting bg-waiting bg-opacity-5 border border-solid border-waiting rounded-lg px-[6px] py-[3px]'>{subfield.value}</span>
											</div>
										);
									})}
							</>
				...
				<div>
					<Timeline className=''>
						...
						>
							<div className='text-white font-normal text-sm leading-[15px]'>
								<p>Executed</p>
								<div className='mt-2 text-text_secondary text-sm'>The transaction will be executed once the threshold is reached.</div>
							</div>
						</Timeline.Item>
					</Timeline>
						...
						{!approvals.includes(userAddress) && (
							<Button
								...
								className={`w-full border-none text-sm font-normal ${
									approvals.includes(userAddress) || (approvals.length === threshold - 1 && !callDataString) ? 'bg-highlight text-text_secondary' : 'bg-primary text-white'
								}`}
							>
								Approve Transaction
							</Button>
