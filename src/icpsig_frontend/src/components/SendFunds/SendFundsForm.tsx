...
import { AutoComplete, Button, Divider, Dropdown, Form, Input, Modal, Spin } from 'antd';
...
import { CircleArrowDownIcon, DeleteIcon, LineIcon, OutlineCloseIcon, SquareDownArrowIcon } from 'src/ui-components/CustomIcons';
...
const SendFundsForm = ({ className, onCancel, defaultSelectedAddress, setNewTxn }: ISendFundsFormProps) => {
	...
	useEffect(() => {
		...
		recipientAndAmount.forEach((item, i) => {
			if (
				item.recipient &&
				(!isValidWeb3Address(item.recipient) || recipientAndAmount.indexOf(recipientAndAmount.find((a) => item.recipient === a.recipient) as IRecipientAndAmount) !== i)
			) {
	...
	const handleSubmit = async () => {
		...
		try {
			...
			const multisigValutTx = (await identityBackend.createTransferTx(activeMultisig, recipients, amounts)).data;
			...
			if (multisigValutTx) {
				...
				//   args: {
				//     address: address,
				//     addresses: getOtherSignatories(
				//       address,
				//       activeMultisig,
				//       multisigAddresses
				//     ),
				//     callHash: multisigValutTx,
												...
												<div className='w-[55%]'>
													...
													>
														<div className='h-[50px]'>
															{recipient && autocompleteAddresses.some((item) => item.value && String(item.value) === recipient) ? (
																<div className='border border-solid border-primary rounded-lg px-2 h-full flex justify-between items-center'>
																	{autocompleteAddresses.find((item) => item.value && String(item.value) === recipient)?.label}
																	<button
																...
																<AutoComplete
																	...
																	options={autocompleteAddresses.filter(
																		(item) => !recipientAndAmount.some((r) => r.recipient && item.value && r.recipient === (String(item.value) || ''))
																	)}
																	id='recipient'
								...
								<div className='flex flex-col gap-y-4'>
									...
									<article className='w-[412px] flex items-center'>
										...
										<p className='p-3 bg-bg-secondary rounded-xl font-normal text-sm text-text_secondary leading-[15.23px] -mb-5'>
											If the recipient account is new, the balance needs to be more than the existential deposit. Likewise if the sending account balance drops below the same
											value, the account will be removed from the state.
										</p>
									</article>
								</div>
							</div>
					...
					<section className='flex items-center gap-x-5 justify-center mt-10'>
						...
						<ModalBtn
							disabled={
								...
								Number(amount) > Number(activeMultisigData?.safeBalance?.toString()) ||
								Object.keys(transactionFields[category].subfields).some(
									(key) => !transactionFieldsObject.subfields[key]?.value && transactionFields[category].subfields[key].required
								)
							}
							loading={loading}
