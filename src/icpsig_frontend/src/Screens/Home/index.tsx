...
const Home = () => {
	const { address, multisigAddresses, activeMultisig, loading, identityBackend, createdAt, addressBook } = useGlobalUserDetailsContext();
	...
	useEffect(() => {
		const handleNewTransaction = async () => {
			if (!activeMultisig || Boolean(!Object.keys(identityBackend).length)) return;
			const identityData = await identityBackend.getMultisigInfoByAddress(activeMultisig);
			if (identityData) {
				setIsOnchain(true);
			} else {
				setIsOnchain(false);
