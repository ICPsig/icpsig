...
import { CopyIcon, UploadBoxIcon } from "@frontend/ui-components/CustomIcons"
  ...
  const props: UploadProps = {
    ...
    beforeUpload: (file) => {
      ...
      reader.onload = (e) => {
        ...
        const uploadedSignatories = JSON.parse(fileContent)?.map(
          (item: string) => item,
        )
  ...
  const ShowSignatories: FC = () => {
    return (
      <>
        ...
        <Modal
          title={
            <h3 className="text-white mb-6 text-lg font-semibold md:font-bold md:text-xl">
              Signatories
            </h3>
          }
          width={600}
        ...
        >
          {uploaded.map((item, i) => (
            ...
            >
              {item}
              <button className="text-primary" onClick={() => copyText(item)}>
    ...
    <div className="flex flex-col">
      ...
      >
        ...
        <p className="ant-upload-text text-white">
          Drag and Drop JSON file to upload
        </p>
