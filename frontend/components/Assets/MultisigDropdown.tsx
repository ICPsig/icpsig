...
const MultisigDropdown: FC<IMultisigDropdownProps> = ({
  className,
  activeAddress,
  setActiveAddress,
}) => {
    ...
    >
      ...
      >
        <p className={classNames("flex items-center")}>
          <span
            className={classNames("ml-[10px] hidden md:inline-flex capitalize")}
          >
            {activeAddress}
          </span>
        </p>
      ...
      >
        <button
          ...
          className={classNames(
            "border-none outline-none shadow-none flex items-center gap-x-4 justify-between rounded-lg p-3 min-w-[190px]",
            {
        ...
        >
          <p className="flex items-center gap-x-[6px]">
            <span
              className={classNames(
                "font-medium text-sm capitalize bg-[#FF79F2] rounded-md py-1 px-2 text-highlight",
              )}
            >
              Proxy
            </span>
          ...
          {activeAddress === "Proxy" ? (
            <OutlineCheckIcon className="text-primary" />
          ) : null}
        </button>
        <button
        ...
        >
          <p className="flex items-center gap-x-[6px]">
            <span
              className={classNames(
                "font-medium text-sm capitalize bg-primary rounded-md py-1 px-2 text-white",
              )}
            >
              Multisig
            </span>
          </p>
          {activeAddress === "Multisig" ? (
            <OutlineCheckIcon className="text-primary" />
          ) : null}
        </button>
      </div>
