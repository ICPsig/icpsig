...
const RenameMultisig = ({ name }: { name: string }) => {
  ...
  const { activeMultisig, setUserDetailsContextState, multisigAddresses } =
  ...
  const handleMultisigNameChange = async () => {
    try {
      ...
      if (
        ...
        const changeNameRes = await fetch(
          `${FIREBASE_FUNCTIONS_URL}/renameMultisigEth`,
          {
            body: JSON.stringify({
        ...
        const { data: changeNameData, error: changeNameError } =
          (await changeNameRes.json()) as { data: any; error: string }
        ...
        if (changeNameData) {
          ...
          const copyObject = copyMultisigAddresses?.find(
            (item) => item.address === multisig.address,
          )
          if (copyObject) {
