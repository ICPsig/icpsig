...
import contactImg from "@frontend/assets/icons/contact-us.svg"
...
import { NotifyMail } from "@frontend/ui-components/CustomIcons"
...
const ContactUs = () => {
  ...
  const handleContactForm = async () => {
    try {
      ...
      const contactFormRes = await fetch(
        `${FIREBASE_FUNCTIONS_URL}/addContactFormResponseEth`,
        {
          body: JSON.stringify({
            email,
            message,
      ...
      const { data: contactFormData, error: contactFormError } =
        (await contactFormRes.json()) as { data: any; error: string }
  ...
  return (
    <div className="bg-bg-main rounded-lg m-auto p-5 h-screen">
      ...
      <p className="text-sm text-text_secondary mt-1 mb-3 ml-[1rem]">
